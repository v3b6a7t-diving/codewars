...
//     const weights = array.reduce((acc, cur)=>{
//         const sum = cur.split('').reduce((a,b) => parseInt(a,10) + parseInt(b,10) ,0);
//         return {...acc, [sum]: cur};
//     }, {});
...
function numberSumm(string) {
    return string.split('').reduce((a,b) => parseInt(a,10) + parseInt(b,10) ,0);
}

function orderWeight(strng) {
    const weights = strng.split(' ').sort((a, b)=>{
        const n1 = numberSumm(a);
        const n2 = numberSumm(b);
        return n1 - n2;
    });
